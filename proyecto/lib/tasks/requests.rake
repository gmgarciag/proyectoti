...
def moverInsumosDespacho sku, cantidad
   ...
   while necesario > 0 do
      ...
      while i <= nAlmacenes 
		...
		if levadura < 2000
		  transaccion = RestClient.put 'http://moto.ing.puc.cl/banco/trx', {:monto => 2016240, :origen => '572aac69bdb6d403005fb04e', :destino => '572aac69bdb6d403005fb040'}.to_json, :content_type => 'application/json'
		  transaccionParseada = JSON.parse transaccion
		  idTrx = transaccionParseada["_id"]
